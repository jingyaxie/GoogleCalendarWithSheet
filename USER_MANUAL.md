...
- **启用状态**：是否启用该课程表（是/否、Yes/No、1/0、true/false）
...
- **老师日历授权ID**：老师的 Google 日历 ID（通常是邮箱地址）
  - 支持的表头名称：`老师日历授权ID`、`老师日历ID`、`Teacher Calendar ID`等
  - 如果为空，系统会尝试使用老师邮箱作为日历ID

- **学生日历授权ID**：学生的 Google 日历 ID（通常是邮箱地址）
  - 支持的表头名称：`学生日历授权ID`、`学生日历ID`、`Student Calendar ID`等
  - 如果为空，系统会尝试使用学生邮箱作为日历ID
...
- **课次**：课程次数（如：第1次、第2次）
  - 支持的表头名称：`课次`、`Lesson`、`课程次数`等

- **日期**：课程日期
  - 支持的表头名称：`日期`、`Date`、`课程日期`等
  - 支持的格式：`2025/11/14`、`2025-11-14`

- **课程内容/主题**：课程主题或内容
  - 支持的表头名称：`课程内容/主题`、`课程内容`、`主题`、`Course Title`等

- **开始时间**：课程开始时间
  - 支持的表头名称：`开始时间`、`Start Time`、`开始`等
  - 支持的格式：`09:00`、`9:00`、`09:00:00`

- **结束时间**：课程结束时间
  - 支持的表头名称：`结束时间`、`End Time`、`结束`等
  - 支持的格式：`11:30`、`11:30:00`

- **老师**：老师姓名
  - 支持的表头名称：`老师`、`Teacher`、`教师`等

- **学生**：学生姓名
  - 支持的表头名称：`学生`、`Student`、`学员`等

#### 自动添加列

- **记录ID**：系统自动生成，用于唯一标识每条记录
  - 首次同步后自动添加到课程表的最后一列
  - 格式：`REC_1762590558439_5970`
  - 用于确保课程表和状态表的一一对应关系
